...
// Convert a dotted OID string into its value bytes (without tag or
// length), following ASN.1 rules.
...
std::vector<AlgorithmPattern> getDefaultPatterns() {
    ...
    P.push_back({
        "SHA family",
        icase_re(R"(\b(?:SHA[-_]?1|SHA[-_]?224|SHA[-_]?256|SHA[-_]?384|SHA[-_]?512)\b|\bsha1sum\b|\bsha256sum\b|\bsha512sum\b)")
    });
    ...
    // Dotted OID strings (ASCII) for RSA/ECC
    ...
    // Representative ECC parameter hex values (ASCII)
